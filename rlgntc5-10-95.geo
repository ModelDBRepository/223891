...
objref parentSections, distals, proximals, ss, whole_cell
parentSections = new Vector()
parentSections.append(0,3,9,10,16,23,33,46,65,70,75,97,111,119,126, 127, 163, 186) //list of sections that subtrees of which are considered distal

distals = new SectionList()

for i = 0, parentSections.size()-1 {
  dend[parentSections.x(i)] distals.subtree()
  dend[parentSections.x(i)] distals.remove()
}

proximals = new SectionList()
proximals.wholetree()
proximals.remove(distals)
proximals.remove()

whole_cell = new SectionList()

for i = 0, 194 {
  dend[i] whole_cell.append()
}
soma whole_cell.append()

//ss = new Shape()
//ss.color_list(distals, 2)